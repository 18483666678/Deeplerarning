...
class GenData:
    def __init__(self, root, img_size):
        ...
        for i in [self.positive_image_dir, self.negative_image_dir, self.part_image_dir]:
            if not os.path.exists(i):
                os.makedirs(i)

        self.img_path = r"F:\CelebA\Img\img_celeba"
        self.anno_box_path = r"F:\CelebA/Anno/list_bbox_celeba.txt"
        self.anno_landmark_path = r"F:\CelebA/Anno/list_landmarks_celeba.txt"

    def run(self, epoch):
        # ann_box_file = open(self.anno_box_path)
        # print(ann_box_file)
        ...
        for _ in range(epoch):
            for i, line in enumerate(open(self.anno_box_path, "r")):
                print(line)
                if i < 2: continue
                strs = line.split()
                print(f"{self.img_path}/{strs[0]}")
                ...
                if iou>0.65:
                    ...
                    _x1_off,_y1_off,_x2_off,_y2_off = _x1 - x1/w,_y1-y1/h,_x2-x2/w,_y2-y2/h
                    ...
                    _x1_off, _y1_off, _x2_off, _y2_off = _x1 - x1 / w, _y1 - y1 / h, _x2 - x2 / w, _y2 - y2 / h
                    negative_label_txt.write(f"{negative_count}.jpg 2 {_x1_off} {_y1_off} {_x2_off} {_y2_off}\n")
                    negative_label_txt.flush()
                    negative_count += 1

                elif iou<0.3:
                    clip_img.save(f"{self.part_image_dir}/{part_count}.jpg")
                    part_label_txt.write(f"{part_count}.jpg 0 0 0 0 0\n")
